...
For three weeks, we've been focusing on chat interfaces - typing prompts, reading responses, iterating. You've learned that LLMs generate plausible continuations rather than looking things up, that they hallucinate, and that their confident tone isn't a reliability signal.

Here's the practical consequence of all that: every time you ask a chat LLM a factual question, you're asking a system that has no sense of truth. Deep down, it doesn't know what that means. It doesn't search a database. It doesn't read papers. It assembles text that fits the pattern of a good answer, whether or not a good answer exists.
...
This doesn't make it impossible that they get things wrong. But it's a meaningful step forward.

**Three kinds of grounding**

Not all grounded tools work the same way, and the distinction matters for how you use them.

**Grounded in your documents.** Tools like Google's **NotebookLM** let you upload your own PDFs, notes, or transcripts. The AI then answers questions based on what you uploaded, citing specific passages. This is the clearest form of grounding - you control the source material, and the tool tells you where in that material it found its answer. It's useful for synthesising across a set of papers you've already collected, pulling out themes from interview transcripts, or getting a quick overview of a document set. I find this very helpful when getting started in a new area.

**Grounded in published literature.** Tools like **Elicit** and **Consensus** search across corpora of academic papers, and in fact even **Google Scholar** now has an AI search interface that does this. They find relevant studies and summarise findings with citations. This is closer to a literature search assistant - useful for discovery and structured extraction, though the coverage and quality vary by field and topic.

**Grounded in web search.** When you ask for it, most leading chatbots can search the web before generating a response. The output includes links to sources. This is the broadest form of grounding but also the noisiest - web sources vary enormously in quality, and the tool's selection of what to cite isn't always transparent. Sometimes it's actually worse than not grounding: the model's training data averages across millions of sources, which can be more reliable than a single website it happens to find.

That said, web search is the only way for models to access anything after their "knowledge cutoff." Without it, you get confident claims like "this citation cannot be correct as it was published in the future" - because the model doesn't know time has moved on.
...
In practice, you might find yourself using more than one type. Discovering papers with Google Scholar's AI mode, then uploading them to NotebookLM for detailed synthesis, then checking a specific claim with a web search. The tools complement each other when used deliberately. When you do combine tools, keep a brief note of what you used and what you verified - this is good practice for your own records, and increasingly a requirement for transparent reporting.

**What grounding actually gives you**

The key feature of grounded tools is **source attribution**. When a grounded tool tells you something, you can (in principle) trace the claim back to a specific passage in a specific document. This is a substantial improvement over traditional chat, where claims float free of any source.
...
They can find relevant passages across a stack of papers. They can't tell you authoritatively which study was well-designed and which had fatal methodological flaws. They can extract what authors claimed, but they can't weigh the evidence the way a researcher reading carefully would (though many will give it a good go if you ask for it). And they can still miss things - papers in your stack that are relevant but don't use the type of language the tool is looking for, or arguments that span multiple sections and don't reduce to a quotable passage.
...
If you don't have time for the full exercise, even uploading 2-3 papers and asking one question will give you a feel for how this works. (NotebookLM can also synthesise papers into other formats - a podcast-style conversation, an infographic. Worth a look if you're curious.)

**Journal prompt**

What (if anything) did the grounded tool surface that you hadn't noticed? What did it miss or flatten? How does this compare to the experience of asking a chat LLM about the same topic?
